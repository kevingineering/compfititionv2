...
export const AddRequest = (userId: string) => async (
  dispatch: ThunkDispatch<{}, {}, RequestDispatchTypes>
) => {
  try {
    dispatch({
      type: REQUEST_LOADING,
      payload: { type: ADD_REQUEST_BUTTON, id: userId },
    });
    const res = await axios.post('/api/friendrequest/' + userId);
    dispatch({ type: ADD_REQUEST, payload: res.data });
    dispatch(SetAlert('Request sent'));
  } catch (error) {
    dispatch({ type: REQUEST_ERROR, payload: ADD_REQUEST_BUTTON + userId });
  }
};
...
export const DeleteRequest = (id: string) => async (
  ...
  try {
    ...
    await axios.delete('/api/friendrequest/' + id);
...
export const AcceptRequest = (id: string) => async (
  ...
  try {
    ...
    const res = await axios.post('/api/friend/' + id);
