import { ThunkDispatch } from 'redux-thunk';
import axios from 'axios';
import { FriendDispatchTypes, EFriendActions } from './types';
import { Dispatch } from 'react';
import { NO_BUTTON, DELETE_FRIEND_BUTTON } from '../buttonTypes';
import { SetAlert } from '../alert/actions';

export const GetFriends = () => async (
  dispatch: ThunkDispatch<{}, {}, FriendDispatchTypes>
) => {
  try {
    dispatch({ type: EFriendActions.FRIEND_LOADING, payload: NO_BUTTON });
    const res = await axios.get('/api/friendship/');
    dispatch({
      type: EFriendActions.GET_FRIENDS,
      payload: res.data,
    });
  } catch (error) {
    dispatch({ type: EFriendActions.FRIEND_ERROR });
  }
};

export const GetDifferentUserInfo = (userId: string) => async (
  dispatch: ThunkDispatch<{}, {}, FriendDispatchTypes>
) => {
  try {
    dispatch({ type: EFriendActions.FRIEND_LOADING, payload: NO_BUTTON });
    const res = await axios.get('/api/friendship/' + userId);
    dispatch({
      type: EFriendActions.GET_DIFFERENT_USER_INFO,
      payload: res.data,
    });
  } catch (error) {
    dispatch({ type: EFriendActions.FRIEND_ERROR });
  }
};

export const DeleteFriend = (friendId: string) => async (
  dispatch: ThunkDispatch<{}, {}, FriendDispatchTypes>
) => {
  try {
    dispatch({
      type: EFriendActions.FRIEND_LOADING,
      payload: DELETE_FRIEND_BUTTON,
    });
    await axios.delete('/api/friendship/' + friendId);
    dispatch({ type: EFriendActions.DELETE_FRIEND });
    dispatch(SetAlert('Friendship deleted', true));
  } catch (error) {
    dispatch({ type: EFriendActions.FRIEND_ERROR });
  }
};

export const FilterFriends = (text: string) => async (
  dispatch: Dispatch<FriendDispatchTypes>
) => {
  dispatch({ type: EFriendActions.FILTER_FRIENDS, payload: text });
};

export const FriendFilterFriends = (text: string) => async (
  dispatch: Dispatch<FriendDispatchTypes>
) => {
  dispatch({ type: EFriendActions.FRIEND_FILTER_FRIENDS, payload: text });
};

export const ClearFilteredFriends = () => async (
  dispatch: Dispatch<FriendDispatchTypes>
) => {
  dispatch({ type: EFriendActions.CLEAR_FILTERED_FRIENDS });
};

export const ClearFriend = () => async (
  dispatch: Dispatch<FriendDispatchTypes>
) => {
  dispatch({ type: EFriendActions.CLEAR_FRIEND });
};

export const SetFriendSelectedGoal = (goalId: string) => (
  dispatch: Dispatch<FriendDispatchTypes>
) => {
  dispatch({ type: EFriendActions.SET_FRIEND_CURRENT_GOAL, payload: goalId });
};
