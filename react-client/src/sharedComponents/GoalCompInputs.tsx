...
import { createFormValuesMap, updateFormValuesMap } from '../util/mapFunctions';
import { dateIsBeforeToday, getGoalTime } from '../util/dateFunctions';
import { EGoalType, TGoal, TCompetition } from '../types';

interface IProps {
  dispatchAction: (goal: TGoalDTO) => void;
  goal?: TGoal;
...
const GoalCompInputs: React.FC<IProps> = ({
  dispatchAction,
  goal,
  competition,
  isGoal,
  ...
  const isStarted = dateIsBeforeToday(formState.inputs.get('startDate')!.value || false):
    goal !== undefined && dateIsBeforeToday(goal.startDate.toString());
    ...
    <div className='form-container'>
      ...
      <form autoComplete='off' className='form'>
        ...
        <Input
          ...
          value={formState.inputs.get('startDate')!.value}
          isValid={formState.inputs.get('startDate')!.isValid}
          handleInput={handleInput}
          validators={[
            VALIDATOR_REQUIRE(),
            VALIDATOR_DATE_NOT_AFTER_2100(
              formState.inputs.get('startDate')!.value
            ),
            ...(isStarted
              ? []
              : [
        ...
        <Input
          ...
          validators={[
            ...
            ...(isStarted
              ? [
                  VALIDATOR_DURATION_NOT_PAST(
                    getGoalTime(goal!.startDate, goal!.duration).time
                  ),
                ]
              : []),
